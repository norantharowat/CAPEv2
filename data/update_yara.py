...
repo = git.Repo('/opt/reversinglabs-yara-rules')
# repo = git.Repo( '/opt/test-update' )


def get_latest_commit():
    online_commits =repo.git.ls_remote("--heads", "origin")
    online_commits =online_commits.partition('refs')[0]
    online_commits = online_commits.strip()

    return online_commits



while True:

    ofline_commit = repo.head.commit 
    online_commit = get_latest_commit()
   

    if( str(online_commit) == str(ofline_commit)):
        pass
    else:
        # repo.git.pull("origin")
        state = repo.git.pull("origin") 
